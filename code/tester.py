...
class Tester:
    def __init__(self, model, loss, CFG, checkpoint, test_loader, dataset_path, show=True):
        ...
        if self.show:
            dir_exists("save_picture\STARE_128182\_SSNet_5%labeled")
            remove_files("save_picture\STARE_128182\_SSNet_5%labeled")
        ...
        with torch.no_grad():
            for i, (img, gt) in enumerate(tbar):
                ...
                if self.show:
                    ...
                    cv2.imwrite(
                        f"save_picture\STARE_128182\_SSNet_5%labeled/img{i}.png", np.uint8(img.cpu().numpy()*255))
                    cv2.imwrite(
                        f"save_picture\STARE_128182\_SSNet_5%labeled/gt{i}.png", np.uint8(gt.cpu().numpy()*255))
                    cv2.imwrite(
                        f"save_picture\STARE_128182\_SSNet_5%labeled/pre{i}.png", np.uint8((1-predict)*255))
                    cv2.imwrite(
                        f"save_picture\STARE_128182\_SSNet_5%labeled/pre_b{i}.png", np.uint8(predict_b*255))
